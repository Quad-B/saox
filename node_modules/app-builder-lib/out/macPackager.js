...
class MacPackager extends _platformPackager().PlatformPackager {
  ...
  pack(outDir, arch, targets, taskManager) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (!hasMas || targets.length > 1) {
        ...
        nonMasPromise = (prepackaged ? Promise.resolve() : _this2.doPack(outDir, path.dirname(appPath), _this2.platform.nodeName, arch, _this2.platformSpecificBuildOptions, targets)).then(() => _this2.packageInDistributableFormat(appPath, _builderUtil().Arch.x64, targets, taskManager));
      }
  ...
  signApp(packContext, isAsar) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const appFileName = `${_this6.appInfo.productFilename}.app`;
      yield _bluebirdLst().default.map((0, _fsExtraP().readdir)(packContext.appOutDir), file => {
        if (file === appFileName) {
          return _this6.sign(path.join(packContext.appOutDir, file), null, null);
        }
      ...
      yield _bluebirdLst().default.map((0, _promise().orIfFileNotExist)((0, _fsExtraP().readdir)(outResourcesDir), []), file => {
        if (file.endsWith(".app")) {
          return _this6.sign(path.join(outResourcesDir, file), null, null);
        } else {
          return null;
        }
      });
